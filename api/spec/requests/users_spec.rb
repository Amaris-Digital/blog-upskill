...
RSpec.describe UsersController, type: :controller do
  ...
  describe "Authentication", type: :request do
    ...
    it "handle login properly" do
      ...
      post(routes[0], params: {email: sample_user[:email], password: sample_user{:password}}, headers:)
      expect(response).to have_http_status(:ok)

      # user login with correct email but wrong password
      post(routes[1], params: {email: sample_user[:email], password: "wrong"}, headers:)
      expect(response).to have_http_status(:unprocessable_entity)
    end
  end
end
